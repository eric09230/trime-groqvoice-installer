...
> 注意：`install.bat` / `install.sh` 安裝的是 **Standard（無滑動）版**，中/英切換是 `ascii_mode`、快速不卡。`apk/` 內另附的 `com.osfans.trime-v3.3.9-27-g04cc8fc8...apk` 是滑動版（中/英切換較慢），一般不建議使用。
...
| 情境 | 方式 | 說明 |
|------|------|------|
| **裝別人的手機（最省事）** | **純手動** | 不用任何工具，傳檔 → 點 APK → 複製 rime → 開權限。詳見 **`手機安裝說明.md`** |
| **自己這台、想自動/重複裝** | **Shizuku** | 用 Shizuku 取得權限後跑 `install-shizuku.sh`（需先裝 Shizuku + aShell You） |
| **用你的手機幫另一台裝** | **Termux 無線 adb** | 在 Termux 跑 `install-wireless.sh`，透過 WiFi 無線偵錯連到對方手機 |

> 提醒：Shizuku / Termux 都要先做一次「無線偵錯配對」才能啟用，所以**一次性幫別人裝，純手動其實步驟最少**。三種方式的詳細步驟分別寫在 `手機安裝說明.md` 和各腳本檔開頭的註解。
